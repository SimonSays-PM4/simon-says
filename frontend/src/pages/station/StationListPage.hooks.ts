...
export const useStationListPage = (): StationReturnProps => {
    ...
    const [stationToDelete, setStationToDelete] = useState<StationDTO>({
        id: 0,
        name: "",
        assemblyStation: false,
        ingredients: []
    });
    ...
    const reloadStations = useCallback(() => {
        ...
        stationService
            ...
            .catch(() => {
                appContext.addNotification(NotificationType.ERR, "Stationen konnte nicht geladen werden");
            })
            .finally(() => {
                setIsLoading(false);
            });
    ...
    const deleteStation = useCallback(() => {
        if (stationToDelete.id && stationToDelete.id > 0) {
            setIsLoading(true);
            stationService
                .deleteStation(eventId, stationToDelete.id)
                .then(() => {
                    setShowDeletePopup(false);
                })
                .catch(() => {
                    appContext.addNotification(NotificationType.ERR, "Station konnte nciht gelöscht werden");
                })
                .finally(() => {
                    setIsLoading(false);
                });
    ...
    return {
        stationList,
        stationActions: { stationToDelete, setStationToDelete, deleteStation },
        isLoading,
        showDeletePopup,
        setShowDeletePopup,
        eventId
